...
_DEFAULT_LEGACY_APP_DIR = LEGACY_APP_DIR
_DEFAULT_LEGACY_STATE_FILE = LEGACY_STATE_FILE
...
def _migrate_legacy_state(config: dict[str, Any]) -> dict[str, Any]:
    legacy_state_file = (
        LEGACY_STATE_FILE
        if LEGACY_STATE_FILE != _DEFAULT_LEGACY_STATE_FILE
        else get_app_dir() / "state.json"
    )
