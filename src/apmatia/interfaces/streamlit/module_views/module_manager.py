"""Schema-selected Streamlit renderer for the Module Manager view."""
...
def _render_module(module: dict[str, object], *, module_index: int, module_count: int) -> bool:
    ...
    is_manager = module_id == "module_manager"
    ...
    with st.container(border=True):
        ...
        with button_col:
            if st.button(
                ...
                use_container_width=True,
                disabled=is_manager,
                help="The Module Manager stays visible so hidden modules remain recoverable." if is_manager else None,
            ):
                return _execute_and_rerun(
...
def _render_view(
    ...
    *,
    ...
    is_manager_view = view_id == "module_manager.module_manager.view"
    ...
    with view_right:
        if st.button(
            ...
            use_container_width=True,
            disabled=is_manager_view,
            help="The Module Manager view stays visible so hidden views remain recoverable." if is_manager_view else None,
        ):
            return _execute_and_rerun(
                "set_view_visibility",
