...
from apmatia.interfaces.streamlit.module_views.portable_page import (
    is_contract_ready_view,
    render_portable_module_view,
)
...
def render() -> None:
    ...
    if is_contract_ready_view(selected_view):
        try:
            document = get_module_view_document(view_id)
        except ApiError as error:
            st.error(f"Unable to load portable view document: {error.detail}")
            return
        render_portable_module_view(document)
        return
...
def _render_legacy_target_controls(
    *,
    spec: object,
    ...
    with st.container(border=True):
        ...
        target_kind = st.radio(
            ...
            options=("agent", "group"),
            horizontal=True,
            key=f"module_view_target_kind:{spec.view_id}",
        )

        form_result = _render_legacy_target_form(
            ...
            initial_values=None,
            form_key=f"module_view_create_form:{spec.view_id}",
        )
    ...
    with st.expander("Create a new group", expanded=False):
        ...
        with st.form(f"module_view_create_group_form:{spec.view_id}"):
...
def _render_legacy_target_form(
    *,
    view_id: str,
    title: str,
    submit_label: str,
    ...
    target_options = _legacy_target_options(target_kind, agents=agents, groups=groups)
    if not target_options:
        st.warning("No chat targets are available yet.")
        return None

    initial_target = _legacy_initial_target(initial_values)
...
def _legacy_initial_target(initial_values: dict[str, object] | None) -> str | None:
    if not isinstance(initial_values, dict):
        return None
