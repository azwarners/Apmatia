...
def source_inspection_tool_definitions() -> list[dict[str, Any]]:
    return [
        {
            "name": "apmatia_tree",
            ...
            "provider_id": SOURCE_INSPECTION_PROVIDER_IDS["tree"],
            "enabled": True,
        ...
        },
        {
            ...
            "output_schema": {"type": "object"},
            "provider_id": SOURCE_INSPECTION_PROVIDER_IDS["read"],
            "enabled": True,
            "confirmation_required": False,
            "read_only": True,
        ...
        },
        {
            ...
            "output_schema": {"type": "object"},
            "provider_id": SOURCE_INSPECTION_PROVIDER_IDS["trace_import"],
            "enabled": True,
            "confirmation_required": False,
            "read_only": True,
...
class SourceInspectionToolProvider:
    provider_id: str
    action: str
    ...
    def execute(self, arguments: dict[str, Any], *, tool_call: Any = None) -> Any:
        try:
            if self.action == "tree":
                return _inspect_tree(str(arguments["root_dir"]), depth=arguments.get("depth"), base_dir=self.base_dir)
            if self.action == "read":
                return _read_source_file(str(arguments["file_path"]), base_dir=self.base_dir)
            if self.action == "trace_import":
                return _trace_imports(str(arguments["module_path"]), base_dir=self.base_dir)
            raise ValueError(f"Unsupported source inspection action: {self.action}")
        except (FileNotFoundError, NotADirectoryError, IsADirectoryError, ValueError) as exc:
            return _error_result("SOURCE_INSPECTION_ERROR", str(exc), action=self.action)
...
def build_source_inspection_tool_providers(base_dir: Path | None = None) -> list[SourceInspectionToolProvider]:
    return [
        SourceInspectionToolProvider(SOURCE_INSPECTION_PROVIDER_IDS["tree"], "tree", base_dir=base_dir),
        SourceInspectionToolProvider(SOURCE_INSPECTION_PROVIDER_IDS["read"], "read", base_dir=base_dir),
        SourceInspectionToolProvider(
            SOURCE_INSPECTION_PROVIDER_IDS["trace_import"],
            "trace_import",
            base_dir=base_dir,
        ),
    ]
...
def _inspect_tree(root_dir: str, *, depth: Any, base_dir: Path | None) -> dict[str, Any]:
    root_path = _resolve_input_path(root_dir, base_dir=base_dir)
...
def _read_source_file(file_path: str, *, base_dir: Path | None) -> dict[str, Any]:
    resolved = _resolve_input_path(file_path, base_dir=base_dir)
...
def _trace_imports(module_path: str, *, base_dir: Path | None) -> dict[str, Any]:
    start_path = _resolve_module_path(module_path, base_dir=base_dir)
    ...
    repo_root = _detect_repo_root(start_path, base_dir=base_dir)
...
def _resolve_input_path(value: str, *, base_dir: Path | None) -> Path:
...
def _resolve_module_path(value: str, *, base_dir: Path | None) -> Path:
    return _resolve_input_path(value, base_dir=base_dir)
