...
AGENT_CONFIG_PROVIDER_IDS = {
    "readme_first": "builtin.agent_config_readme_first",
    "tree": "builtin.agent_config_tree",
    "read": "builtin.agent_config_read",
}
...
def agent_config_tool_definitions() -> list[dict[str, Any]]:
    return [
        {
            "name": "agent_config_readme_first",
            "description": (
                "Read this first. Return the exact knowledge root location, accepted path aliases, "
            ...
            "provider_id": AGENT_CONFIG_PROVIDER_IDS["readme_first"],
            "enabled": True,
        ...
        },
        {
            ...
            "description": (
                "Return a JSON tree for the knowledge root using paths relative to that root "
                "with optional depth and file visibility controls. Paths like '.', '/', '/knowledge', "
                "'/knowledge/docs', or 'docs' are normalized to the same root."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
            ...
            "output_schema": {"type": "object"},
            "provider_id": AGENT_CONFIG_PROVIDER_IDS["tree"],
            "enabled": True,
            "confirmation_required": False,
            "read_only": True,
        ...
        },
        {
            ...
            "description": "Read a UTF-8 file from the knowledge root using a path relative to the knowledge root. '/', '/knowledge', and '/knowledge/...' are normalized for convenience.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "file_path": {
            ...
            "output_schema": {"type": "object"},
            "provider_id": AGENT_CONFIG_PROVIDER_IDS["read"],
            "enabled": True,
            "confirmation_required": False,
            "read_only": True,
    ...
    def execute(self, arguments: dict[str, Any], *, tool_call: Any = None) -> Any:
        try:
            if self.action == "readme_first":
                return _agent_config_info(base_dir=self.base_dir)
            if self.action == "tree":
                return _inspect_agent_config_tree(
            ...
            if self.action == "read":
                return _read_agent_config_file(str(arguments["file_path"]), base_dir=self.base_dir)
...
def build_agent_config_tool_providers(base_dir: Path | None = None) -> list[AgentConfigToolProvider]:
    return [
        AgentConfigToolProvider(AGENT_CONFIG_PROVIDER_IDS["readme_first"], "readme_first", base_dir=base_dir),
        AgentConfigToolProvider(AGENT_CONFIG_PROVIDER_IDS["tree"], "tree", base_dir=base_dir),
        AgentConfigToolProvider(AGENT_CONFIG_PROVIDER_IDS["read"], "read", base_dir=base_dir),
    ]


def agent_config_base_dir(base_dir: Path | None = None) -> Path:
    return Path(base_dir or get_app_dir()).expanduser()


def agent_config_root(base_dir: Path | None = None) -> Path:
    return agent_config_base_dir(base_dir) / "knowledge"


def _agent_config_info(*, base_dir: Path | None) -> dict[str, Any]:
    root = agent_config_root(base_dir)
    return {
        "ok": True,
        "base_dir": str(agent_config_base_dir(base_dir)),
        "knowledge_root": str(root),
        "root_aliases": [".", "/", "/knowledge", "knowledge", "knowledge/", "/knowledge/"],
        "accepted_path_form": "Relative to the knowledge root. Leading /knowledge or / are normalized to the knowledge directory.",
        "tools": [
            {
                "name": "agent_config_readme_first",
        ...
        "examples": [
            {"tool": "agent_config_tree", "args": {"path": "."}},
            {"tool": "agent_config_tree", "args": {"path": "/knowledge", "mode": "directories_and_files", "depth": 3}},
            {"tool": "agent_config_read", "args": {"file_path": "/knowledge/README.md"}},
        ],
        "error": None,
    }


def _inspect_agent_config_tree(path: str, *, depth: Any, mode: str, base_dir: Path | None) -> dict[str, Any]:
...
def _read_agent_config_file(file_path: str, *, base_dir: Path | None) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "base_dir": str(agent_config_base_dir(base_dir)),
        "knowledge_root": str(agent_config_root(base_dir)),
...
def _resolve_knowledge_path(value: str, *, base_dir: Path | None) -> tuple[Path, str]:
    root = agent_config_root(base_dir).resolve()
