...
from .services import TopicManagementService, get_discussion
...
class ApmatiaTopicManagementModuleViewProvider:
    ...
    def execute_command(
        ...
        *,
        ...
        if command.command_id == "discuss.discussion.open":
            discussion_id = str(payload.get("discussion_id") or "").strip()
            if not discussion_id:
                raise ValueError("discussion_id is required.")
            if get_discussion(discussion_id, bundle=self._service.bundle) is None:
                raise ValueError(f"Discussion not found: {discussion_id}")
            return {"status": "opened", "discussion_id": discussion_id}

        if command.command_id == "discuss.discussion.create":
            title = str(payload.get("title") or "New Discussion").strip() or "New Discussion"
            topic = self._service.create_topic(Topic(title=title, owner_user_id=context.user_id))
            discussion = self._service.create_discussion(
                Discussion(topic_id=topic.id, title=title, owner_user_id=context.user_id)
            )
            agent_id = payload.get("agent_id")
            if agent_id not in (None, ""):
                self._service.create_participant(
                    DiscussionParticipant(discussion_id=discussion.id, agent_id=int(agent_id), owner_user_id=context.user_id)
                )
            return {"status": "created", "discussion_id": discussion.id, "topic_id": topic.id}
