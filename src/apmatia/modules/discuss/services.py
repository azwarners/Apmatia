...
def _resolve_discuss_db(data_dir: Path) -> Path:
    database = data_dir / "discuss.db"
    legacy_database = data_dir / "contacts_and_discussions.db"
    if database.exists() or not legacy_database.exists():
        return database
    return legacy_database


DISCUSS_DB = _resolve_discuss_db(DATA_DIR)
...
class TopicManagementService:
    def __init__(self, bundle: TopicManagementBundle | None = None):
        self.bundle = bundle or TopicManagementBundle(DISCUSS_DB)
...
def get_discussion(discussion_id: str | int, *, bundle: TopicManagementBundle | None = None) -> Discussion | None:
    ...
    bundle = bundle or TopicManagementBundle(DISCUSS_DB)
