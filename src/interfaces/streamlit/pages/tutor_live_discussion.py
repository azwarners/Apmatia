...
from src.interfaces.streamlit.api_client import ApiError, discussion_state, get_wiki_tree, list_agents, prompt_discussion, stop_discussion
from src.interfaces.streamlit.pages import discussion as discussion_page
from src.interfaces.streamlit.pages.tutor_shared import build_tutor_wiki_context, ensure_selected_node, get_tutor_selected_agent_id
    ...
    if initial_is_streaming:
        ...
        if getattr(fragment_factory, "__module__", "").startswith("streamlit"):
            ...
            def _streaming_fragment() -> dict[str, object]:
                current_snapshot = discussion_page._render_streaming_messages(username=username, agent_name=agent_name)
