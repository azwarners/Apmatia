...
from src.core.model_management_runtime import get_llm_config_manager
...
from src.lib.agent_management.agent_prompt import compile_agent_system_prompt, default_agent_prompt
...
from src.lib.discussions.discussion_templates import DISCUSSION_CHAT_TEMPLATE, build_chat_messages
...
from src.lib.llama_server.log_parser import parse_llama_server_log_file, parse_llama_server_log_turns
...
class DiscussionState:
    ...
    def _build_group_chat_prompt(
        ...
        *,
        ...
        from src.lib.discussions.group_chat import build_turn_prompt
    ...
    def snapshot(self, user_id: int, member_group_ids: set[int]) -> DiscussionSnapshot:
        ...
        if turns and assistant_messages:
            for message, turn in zip(assistant_messages, turns[-len(assistant_messages) :]):
                ...
                if not self._llama_server_status_has_renderable_metrics(
                    llama_status if isinstance(llama_status, dict) else None
                ):
