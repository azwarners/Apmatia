...
from apmatia.modules.agent_loops.commands import COMMAND_DESCRIPTORS
...
from apmatia.modules.agent_loops.module import APMATIA_AGENT_LOOPS_MODULE, register
...
from apmatia.modules.agent_loops.runner import AgentLoopRuntime, LoopTaskRequest
...
def test_tool_manager_tool_executor_executes_builtin_list_agents_without_name_errors(monkeypatch):
    class _Agent:
        id = 7
        name = "Karen Smith"
        owner_user_id = 1
    ...
    class _Result:
        def __init__(self) -> None:
            self.status = "success"
            self.result = {"agents": [{"id": 1, "name": "Ada"}]}
            self.error = None
            self.metadata = {"tool_id": 17, "read_only": True}
    ...
    task = AgentLoopTask(
        ...
        contact_id=1,
        title="List agents",
        prompt="List all Apmatia agents",
        agent_id=7,
    )
    context = type("Context", (), {"task": task})()
    executor = service_module.ToolManagerToolExecutor()
    ...
    result = executor.execute(
        type("Request", (), {"tool_name": "list_agents", "arguments": {}, "call_id": "call_1"})(),
        context,
        EventCancellationToken(),
    )

    assert result.status == "success"
    assert result.output == {"agents": [{"id": 1, "name": "Ada"}]}
    assert result.metadata["tool_id"] == 17
    assert tool_manager.last_call.requester_agent_id == 7
...
def test_module_view_provider_lists_contacts_and_stop_command(tmp_path: Path, monkeypatch):
    ...
    from apmatia.modules.agent_loops import module_views as module_views_module
...
def test_file_agent_loop_task_repository_ignores_empty_task_files(tmp_path: Path):
    from apmatia.modules.agent_loops.repository import FileAgentLoopTaskRepository
