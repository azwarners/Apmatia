...
def _service_module():
    import apmatia.modules.ai_host_management.services as services
...
def test_resource_view_lists_registered_hosts(monkeypatch):
    import apmatia.modules.ai_host_management.module_views as module_views
    import apmatia.modules.ai_host_management.views as views
    from apmatia.modules.ai_host_management.models import AIHostResourceReport
...
monkeypatch.setattr(
    ...
    resource_view = next(view for view in views.VIEW_DESCRIPTORS if view.view_id == "ai_host_management.resources.view")
...
def test_host_view_exposes_create_action_payload():
    ...
    views = importlib.reload(views)
    host_view = next(view for view in views.VIEW_DESCRIPTORS if view.view_id == "ai_host_management.hosts.view")
    create_action = next(action for action in host_view.metadata["ui"]["view_actions"] if action["intent"] == "create")

    assert create_action["payload"]["command_id"] == "ai_host_management.hosts.create"
