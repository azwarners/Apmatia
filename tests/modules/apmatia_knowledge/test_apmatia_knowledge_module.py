...
from apmatia.modules.apmatia_knowledge.actions import ACTION_DESCRIPTORS
from apmatia.modules.apmatia_knowledge.commands import COMMAND_DESCRIPTORS
from apmatia.modules.apmatia_knowledge.module import APMATIA_KNOWLEDGE_MODULE, register
from apmatia.modules.apmatia_knowledge.module_views import ApmatiaKnowledgeModuleViewProvider
from apmatia.modules.apmatia_knowledge.views import VIEW_DESCRIPTORS
...
def test_apmatia_knowledge_module_registers_agent_config_view():
    registry = Registry()

    register(registry)

    assert registry.list_modules() == [APMATIA_KNOWLEDGE_MODULE]
...
def test_apmatia_knowledge_module_view_provider_updates_agent_roots_with_warnings(tmp_path: Path):
    workspace_root = tmp_path / "workspace" / "planner"
    workspace_root.mkdir(parents=True)
    knowledge_root = tmp_path / "workspace" / "shared-knowledge"
    manager = _AgentManager(
        [
            Agent(id=1, name="Planner", workspace_root="", knowledge_root=""),
        ]
    )
    provider = ApmatiaKnowledgeModuleViewProvider(agent_manager=manager)
