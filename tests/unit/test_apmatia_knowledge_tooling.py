...
from apmatia.modules.apmatia_knowledge.tooling import (
    build_knowledge_tool_providers,
...
@pytest.fixture
def knowledge_tool_manager(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.chdir(tmp_path)
    knowledge_root = tmp_path / "workspace" / "knowledge"
    knowledge_root.mkdir(parents=True)
    return ToolManager(
        InMemoryToolDefinitionRepository(),
        InMemoryAssignmentRepository(),
        InMemoryAgentService(),
        builtin_providers=build_knowledge_tool_providers(base_dir=tmp_path),
        builtin_definitions=knowledge_tool_definitions(),
    )
...
def test_knowledge_tools_are_registered(knowledge_tool_manager: ToolManager):
    names = {tool.name for tool in knowledge_tool_manager.list_tool_definitions()}

    assert {"apmatia_knowledge_readme_first", "apmatia_knowledge_tree", "apmatia_knowledge_read"} <= names


def test_knowledge_readme_first_returns_root_and_usage(knowledge_tool_manager: ToolManager, tmp_path: Path):
    tool_id = _assign_tool(knowledge_tool_manager, 1, "apmatia_knowledge_readme_first")
    result = knowledge_tool_manager.execute_tool_call(
    ...
    assert result.result["knowledge_root"] == str(tmp_path / "workspace" / "knowledge")
    assert "/knowledge" in result.result["root_aliases"]
    assert any(tool["name"] == "apmatia_knowledge_read" for tool in result.result["tools"])
    assert any(example["tool"] == "apmatia_knowledge_read" for example in result.result["examples"])


def test_knowledge_tree_can_filter_files_and_respect_depth(
    knowledge_tool_manager: ToolManager,
    tmp_path: Path,
):
    root = tmp_path / "workspace" / "knowledge"
    docs = root / "docs"
    ...
    tool_id = _assign_tool(knowledge_tool_manager, 1, "apmatia_knowledge_tree")
    result = knowledge_tool_manager.execute_tool_call(
        ToolCall(
            tool_id=tool_id,
...
def test_knowledge_read_returns_text_and_metadata(
    knowledge_tool_manager: ToolManager,
    tmp_path: Path,
):
    root = tmp_path / "workspace" / "knowledge"
    ...
    tool_id = _assign_tool(knowledge_tool_manager, 1, "apmatia_knowledge_read")
    result = knowledge_tool_manager.execute_tool_call(
...
def test_knowledge_tools_reject_path_escape(
    knowledge_tool_manager: ToolManager,
):
    tool_id = _assign_tool(knowledge_tool_manager, 1, "apmatia_knowledge_read")
    result = knowledge_tool_manager.execute_tool_call(
